...
Next action: When the detached team100 run finishes: file scratchpad team100-core.md/json and team100-ref.md/json into perf/ as team100.<sha>.*, write the 1.5 findings (fold-cost curve crossings, RSS, sessions[]-axis) into the perf README and HOTPATH, log the quadratic session lookup (fold.rs/fold.ts `sessions.find` per event) as an idea with predicted gain — never built here — and mark 1.5 done. User actions pending: push rust-core (PR #2 CI on the fixes) and merge wave-a (`git merge 779db6f`) so memory-lead 1.4 can wire quote (schema regen, DecisionState, status/decisions render) and then take D3+D4 together in one digest re-record after the D4 sha lands.
