...
## Phase 2 — Seam and providers [pending] — 2/4 done
...
Next action: Start 2.4 (enrichment event type in packages/schema with producer, model, question id, answer, subject event id; fold ignores for state, index reads) before 2.3, since the cloud provider needs somewhere to land stored judgements; 2.3 then wires the api.sofar.sh client under the sync client's auth.
