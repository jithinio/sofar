...
Next action: Author the 3.1 verification contract as a Decision (persisted, restart-safe, invalidates on changed inputs) and build it; or cut the RC (4.2) first if the operator prefers — the RC checklist is in 4.2's title (both D18 gate tables in its note, SOFAR_LESSONS=off documented). 4.1 waits for bench-refresh 4.1.
