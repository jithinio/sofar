...
Next action: Nothing here: the record is closed and reopening happens by working on it again with sofar switch. The one live obligation is elsewhere — do not publish sofar.sh until commit-attribution finishes, since this initiative's protocol-block change ships in that same release (repo memory M7).
