...
Next action: Nothing is outstanding: the remaining act is closing the record with sofar close, once the user is satisfied. Do NOT publish sofar.sh first — repo memory M7 rules that this initiative's protocol-block change ships in the same release as commit-attribution, which is still unfinished.
