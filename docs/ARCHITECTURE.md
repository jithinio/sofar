...
| `core/index-tier1.ts` | **Keyed tier.** The decision-scope half (memory-lead 2.1, D6): every decision that guards or names a file, with supersession mirrored from the fold. Also derived relevance: who else touched a path, and from which initiative. |
