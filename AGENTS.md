...
1. BEFORE any work: run `harness status` and orient from the record in
   `.harness/` (detail in `.harness/initiatives/<slug>/plan.md` and
   `decisions.md`; authoritative contracts in `docs/SPEC.md`). Do not ask
   the user for context the record already answers. (`docs/harness.md` is
   the archived pre-migration record — history only, never written.)
