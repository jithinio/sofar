...
mod tests {
    ...
    /// `Number::toString` parity on exact ties (…402.78125 prints `7812`, the
    /// even digit) and around them: every case generated by Node.
    #[test]
    #[allow(
        clippy::unreadable_literal,
        reason = "bit patterns generated by Node, compared as is"
    )]
    fn number_to_string_breaks_ties_as_js_does() {
        const CASES: &[(f64, &str)] = &include!("../tests/fixtures/js-number-cases.rs.in");
        let cases = CASES;
        assert!(cases.len() > 1000);
        for (x, want) in cases {
            assert_eq!(number_to_string(*x), *want, "{:#x}", x.to_bits());
        }
    }
