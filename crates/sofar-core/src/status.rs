...
/// What `sofar status` knows about the record's other copies
/// (branch-visibility D1): the provenance of a union fold and the home dir
/// its worktree paths are tildified against. Default: no other copy.
#[derive(Debug, Clone, Copy, Default)]
pub struct CopiesView<'a> {
    pub provenance: Option<&'a crate::record_copies::RecordProvenance>,
...
pub fn render_full_status(
    ...
    if let Some(p) = copies.provenance {
        lines.extend(crate::record_copies::render_provenance_block(
            p,
            copies.home,
        ));
    }
...
mod tests {
    ...
    fn empty_record_renders_the_bare_block() {
        ...
        let full = render_full_status(&state, true, None, &CopiesView::default());
        assert_eq!(
            full,
