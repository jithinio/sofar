...
Re-recorded on **release/0.34.0-rc.3** (from main bf916b73, carrying the
rust-core merge d039a77c) from the TypeScript reference. The version bump
is the whole diff: two goldens changed, each by one string. `argv.fast-path`
(`--version` is now `0.34.0-rc.3`) and `open.O2-update-segment` (`you have
0.34.0-rc.3`), both previously `0.33.0-rc.2+trunk`. Every other golden is
